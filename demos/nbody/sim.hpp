...
class GravSim {
 ...
 public:
  ...
  GravSim(size_t n_bodies, std::vector<particle_data<num_t>>&& particles)
      : GravSim(n_bodies) {
    m_coulomb_charges_buf =
        std::unique_ptr<SyclBufs<num_t>>(new SyclBufs<num_t>(n_bodies));

    auto accs = m_bufs.write().gen_host_write_accs(write_bufs_t<0, 1>{});
    auto charge_acc =
        m_coulomb_charges_buf->gen_host_write_accs(write_bufs_t<0>{});
        ...
        case force_t::COULOMB: {
          if (!m_coulomb_charges_buf) {
            throw std::runtime_error(
                "Coulomb charge buffer wasn't initialized!");
          }

          // Read accessor for particle charges
          auto charges_acc = std::get<0>(
